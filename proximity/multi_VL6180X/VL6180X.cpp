...
void VL6180X::configureDefault(void)
{
  ...
  // Period = (10 + 10*n)
  writeReg(SYSRANGE__INTERMEASUREMENT_PERIOD, 0x00);

  // sysals__intermeasurement_period = 0 (10 ms)
  // Period = (10 + 10*n)
  writeReg(SYSALS__INTERMEASUREMENT_PERIOD, 0x00);

  // als_int_mode = 0 ([5:3])(ALS disabled); range_int_mode = 4 ([2:0])(range new sample ready interrupt)
  writeReg(SYSTEM__INTERRUPT_CONFIG_GPIO, 0x04);


  // Reset other settings to power-on defaults

  // sysrange__max_convergence_time = 5 (5 ms)
  writeReg(VL6180X::SYSRANGE__MAX_CONVERGENCE_TIME, 0x05);
